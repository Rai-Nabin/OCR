...
def detect_text_cooridinates(image_path):
    ...
    """
    ocr = PaddleOCR(det_model_dir='./model/det_model/ch_PP-OCRv3_det_slim_infer/')
...
def draw_bounding_box_and_save(image, bounding_boxes, FILE_NAME):
    ...
    for index, box in enumerate(formatted_bounding_boxes):
        ...
        cv.imwrite(f"output/{FILE_NAME}.jpg", image)
